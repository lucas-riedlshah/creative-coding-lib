...
export class Graph2D {
    ...
    get adjacency_matrix() { return this._adjacency_matrix; }
...
export function shortest_cycles(graph) {
    const cycles = [];
    const adjacency_matrix_clone = graph.adjacency_matrix.map(row => row.map(w => w)); // deep-clone the matrix
    let next_cycle = find_cycle();
    ...
    function find_cycle() {
        const cycle = [];
        const start_index = find_start_vertex_index();
        if (start_index === null)
            return null;
        let current_index = start_index;
        let next_index = get_out_neighbour_indices(current_index)[0];
        ...
        while (next_index !== start_index) {
            ...
            const out_neighbour_indices = get_out_neighbour_indices(next_index)
     ...
     */
    function check_cycle_validity(cycle) {
        ...
        if (cycle.length !== graph.vertices.length) {
            ...
            for (const v of graph.vertices)
                if (!cycle.includes(v) && cycle_polygon.contains(v))
                    return false;
        }
        // Check for circuits
        ...
        for (const other of cycles) {
            if (cycle === other || cycle.length !== other.length)
                continue;
            is_duplicate = true;
            for (const vertex of cycle) {
                if (other.includes(vertex))
                    continue;
                is_duplicate = false;
                break;
            }
    ...
    function find_start_vertex_index() {
        const start_index = null;
    ...
    function get_out_neighbour_indices(vertex_index) {
