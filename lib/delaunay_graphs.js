...
export function generate_voronoi_graph(points) {
    ...
    return graph;
    function edges_of_triangle(triangle_n) {
        return [3 * triangle_n, 3 * triangle_n + 1, 3 * triangle_n + 2];
    }
    function triangle_n_of_edge(edge_n) {
    ...
    function circumcenter(triangle_vertices) {
        ...
        return new Vector2(1 / D * (a_squared_magnitude * (b.y - c.y) + b_squared_magnitude * (c.y - a.y) + c_squared_magnitude * (a.y - b.y)), 1 / D * (a_squared_magnitude * (c.x - b.x) + b_squared_magnitude * (a.x - c.x) + c_squared_magnitude * (b.x - a.x)));
    }
...
export function generate_urquhart_graph(points) {
    const delaunay = Delaunator.from(points, (point) => point.x, (point) => point.y);
    ...
    for (let i = 0; i < delaunay.triangles.length; i += 3) {
        ...
        const edges = [[vertex_a, vertex_b], [vertex_b, vertex_c], [vertex_c, vertex_a]];
        edges.sort((edge_x, edge_y) => Vector2.distance(...edge_x) - Vector2.distance(...edge_y));
        graph.remove_edge(...edges[2]);
    }
    return graph;
}
function delaunator_to_graph_2d(points, delaunator) {
    const graph = new Graph2D(points);
